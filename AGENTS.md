...
- `src/routes/api/forecasts/run/+server.ts`: Authenticated, brand-scoped JSON endpoint that runs exactly one forecast model for one brand and horizon (per-model isolation).
...
- `src/routes/forecasts/history-days.server.ts`: Route-local server helper that resolves a brand's days of usable sales history for the controls bar (`0` = no sales data, `null` = unknown/no brand).
