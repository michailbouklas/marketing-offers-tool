<script lang="ts">
  ...
  import type {
    GeneratedImageFilterOption,
    GeneratedImagesHistoryPage,
    GeneratedImagesPromptGroup,
  ...
  let {
    ...
    brands,
    basePath,
    usageEndpoint = "/api/images/usage",
    chartDescription = "Images you generated, by day.",
    emptyMessage = "You have not generated any images yet.",
  }: {
    imagePage: GeneratedImagesHistoryPage;
    promptGroups: GeneratedImagesPromptGroup[];
    filterOptions: {
      models: GeneratedImageFilterOption[];
      providers: GeneratedImageFilterOption[];
    };
  ...
  let selectedBrandKeys = $state<string[]>([...filters.brand]);
  ...
  $effect(() => {
    selectedBrandKeys = [...filters.brand];
  });
  ...
  const images = $derived(imagePage.items);
  const isPromptView = $derived(filters.view === "prompt");
  const modelOptions = $derived(filterOptions.models);
  const providerOptions = $derived(filterOptions.providers);
  ...
  const page = $derived(imagePage.page);
  const pageSize = $derived(imagePage.pageSize);
  const totalItems = $derived(imagePage.totalItems);
  const totalPages = $derived(imagePage.totalPages);
  const appliedBrandKeys = $derived(filters.brand);
...
{#if hasGeneratedImages}
  <UsageOverTimeChart endpoint={usageEndpoint} description={chartDescription} />
{/if}
    ...
    >
      ...
      >
        ...
        <div class="flex gap-2">
          <Button type="submit" name="view" value={filters.view ?? "table"}>
            Apply
          </Button>
          <Button
            href={basePath}
          ...
          <section class="overflow-hidden rounded-lg border">
            <div class="bg-muted/40 border-b p-4">
              ...
              >
                ...
                <div class="flex flex-wrap gap-2">
                  <Badge variant="secondary">{group.items.length} images</Badge>
