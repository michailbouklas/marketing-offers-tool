...
/**
 * Env-free object-store core: the drivers, types, and key guard, with no
 * dependency on SvelteKit's `$env` virtual modules. The SvelteKit side keeps
 * using ./object-store.server.ts (which re-exports everything here and adds
 * the env-driven `getObjectStore()`); code under src/lib/server/mastra
 * imports this module directly so the standalone `mastra dev` playground can
 * still bundle it.
 */
