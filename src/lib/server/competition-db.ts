...
/**
 * Helpers for querying the competition-scraper ClickHouse replica. The replica
 ...
 *
 * All replica tables are `ReplacingMergeTree(_version) ORDER BY (id)`; queries
 * must use `FINAL` to read deduplicated rows. `_sign` / `_version` are
 * MATERIALIZED columns and never appear in `SELECT *` results.
 */

const DEFAULT_COMPETITION_DATABASE = "competition_scraper_replica";
