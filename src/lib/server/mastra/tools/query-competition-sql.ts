...
import { z } from "zod";
import { getCompetitionClickhouseEnv } from "../env";
...
/**
 * Keeps the agent inside the aggregator-scraper replica: the connection's
 * default database already scopes unqualified names, and qualified references
 * to ClickHouse's own metadata schemas are rejected outright.
 */
...
export type CompetitionSqlResult =
  | {
      ok: true;
      rowCount: number;
...
const globalForCompetitionSql = globalThis as typeof globalThis & {
  competitionSqlClient?: ClickHouseClient;
};
 ...
 * connection defaults to the aggregator-scraper replica database and runs
 * every query with ClickHouse-native read-only enforcement.
 */
function getClient(): ClickHouseClient | null {
  if (globalForCompetitionSql.competitionSqlClient) {
    return globalForCompetitionSql.competitionSqlClient;
  }

  const env = getCompetitionClickhouseEnv();
  ...
  globalForCompetitionSql.competitionSqlClient = createClient({
    url,
    username,
    password,
...
export function validateReadOnlySql(
  ...
  if (forbiddenSchema) {
    return {
      ok: false,
      error: `Forbidden schema reference "${forbiddenSchema[0].trim()}" — query only the competition tables (unqualified names).`,
    };
  }
...
export async function runReadOnlyCompetitionQuery(
  sql: string,
): Promise<CompetitionSqlResult> {
  const validation = validateReadOnlySql(sql);

  if (!validation.ok) {
    console.warn(`[competition-sql] rejected: ${validation.error}\n${sql}`);
  ...
  if (!client) {
    return {
      ok: false,
      error:
        "The competition database is not configured (CLICKHOUSE_URL is missing).",
    };
  }

  console.log(`[competition-sql] query:\n${validation.sql}`);
  ...
  try {
    ...
    console.log(
      `[competition-sql] ${limited.length} row(s) in ${Date.now() - startedAt}ms${
        truncated ? " (truncated)" : ""
      }`,
    );
    ...
    console.error(
      `[competition-sql] failed in ${Date.now() - startedAt}ms: ${message}`,
    );
    return { ok: false, error: `Query failed: ${message}` };
  }
}

export const queryCompetitionSql = createTool({
  id: "query-competition-sql",
  description:
    "Run a single read-only ClickHouse SELECT query against the aggregator-scraper replica " +
    "(tables aggregator, restaurant, offer, offer_snapshot, product, product_price, " +
    "restaurant_category, scrape_session). " +
    ...
    sql: z
      ...
      .describe(
        "A single ClickHouse SELECT statement. Use unqualified table names " +
          ...
          "each table alias, e.g. SELECT count() FROM offer AS o FINAL.",
      ),
  }),
  execute: async ({ sql }) => {
    return runReadOnlyCompetitionQuery(sql);
  },
});
