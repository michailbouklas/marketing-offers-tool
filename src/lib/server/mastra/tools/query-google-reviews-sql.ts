...
import { z } from "zod";
import { getGoogleReviewsClickhouseEnv } from "../env";
...
/**
 * Keeps the agent inside the reviews replica: the connection's default
...
export type GoogleReviewsSqlResult =
  | {
      ok: true;
      rowCount: number;
...
const globalForGoogleReviewsSql = globalThis as typeof globalThis & {
  googleReviewsSqlClient?: ClickHouseClient;
};
 ...
 * connection defaults to the google-maps-scraper replica database and runs
 * every query with ClickHouse-native read-only enforcement.
 */
function getClient(): ClickHouseClient | null {
  if (globalForGoogleReviewsSql.googleReviewsSqlClient) {
    return globalForGoogleReviewsSql.googleReviewsSqlClient;
  }

  const env = getGoogleReviewsClickhouseEnv();
  ...
  globalForGoogleReviewsSql.googleReviewsSqlClient = createClient({
    url,
    username,
    password,
...
export function validateReadOnlySql(
  ...
  if (forbiddenSchema) {
    return {
      ok: false,
      error: `Forbidden schema reference "${forbiddenSchema[0].trim()}" — query only the Google reviews tables (unqualified names).`,
    };
  }
...
/**
 * Numify by declared column type only: counts/sums arrive as strings to
 * preserve precision, and numbers are friendlier for the model. String
 * columns are never touched — crucially `cid`/`business_cid`, which are
 * 19-digit numeric-looking Strings that must stay strings.
 */
...
export async function runReadOnlyGoogleReviewsQuery(
  sql: string,
): Promise<GoogleReviewsSqlResult> {
  const validation = validateReadOnlySql(sql);

  if (!validation.ok) {
    console.warn(`[google-reviews-sql] rejected: ${validation.error}\n${sql}`);
  ...
  if (!client) {
    return {
      ok: false,
      error:
        "The Google reviews database is not configured (CLICKHOUSE_URL is missing).",
    };
  }

  console.log(`[google-reviews-sql] query:\n${validation.sql}`);
  ...
  try {
    ...
    console.log(
      `[google-reviews-sql] ${limited.length} row(s) in ${Date.now() - startedAt}ms${
        truncated ? " (truncated)" : ""
      }`,
    );
    ...
    console.error(
      `[google-reviews-sql] failed in ${Date.now() - startedAt}ms: ${message}`,
    );
    return { ok: false, error: `Query failed: ${message}` };
  }
}

export const queryGoogleReviewsSql = createTool({
  id: "query-google-reviews-sql",
  description:
    "Run a single read-only ClickHouse SELECT query against the Google reviews replica " +
    "(tables businesses, reviews, review_summaries, review_categories, business_features, " +
    "operating_hours, ordering_options, review_category_metrics_timeseries). " +
    "Every table is ReplacingMergeTree — ALWAYS add FINAL after the table alias. " +
    ...
    sql: z
      ...
      .describe(
        "A single ClickHouse SELECT statement. Use unqualified table names " +
          "(the connection selects the replica database) and append FINAL after " +
          "each table alias, e.g. SELECT count() FROM reviews AS r FINAL.",
      ),
  }),
  execute: async ({ sql }) => {
    return runReadOnlyGoogleReviewsQuery(sql);
  },
});
