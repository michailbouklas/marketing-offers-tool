...
import { templateUpdateSchema } from "$lib/services/image-generator/composer-library";
import {
  deleteTemplate,
  updateTemplate,
...
export const PATCH: RequestHandler = async (event) => {
  ...
  const item = await updateTemplate(user.id, event.params.id, parsed.data);
  return json({ item });
};

export const DELETE: RequestHandler = async (event) => {
  const { user } = requireAuthenticatedApiUser(event);
  await deleteTemplate(user.id, event.params.id);
