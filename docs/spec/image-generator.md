...
| `POST /api/images/enhance`             | Body: `{ prompt, brandGuidelines? }`. Returns `{ critique?: string, clarifyingQuestions?: { question: string; example?: string }[], enhancedPrompt?: string }`. Uses OpenAI chat completions; brand guidelines steer questions/critique.            |
