...
- **Enhance prompt** checkbox (default on): when enabled and the request is not coming from "Edit → re-prompt with reference", the client calls `POST /api/images/enhance` first, passing the active brand's guidelines (when a brand is selected) so questions and critique are brand-aware. The enhancement endpoint may return clarifying questions; the UI surfaces them **inline directly under the prompt field** (not at the bottom of the form) — each as a dedicated input with an example answer as helper text, and the user can answer any subset or skip. When the prompt is weak the response also carries a short `critique` shown above the questions explaining what may produce a poor result. The accepted enhanced prompt becomes the prompt sent to generation. When disabled (or when editing with a reference image), the raw prompt is sent as-is.
...
2. **Optional prompt enhancement**: when "Enhance prompt" is on, `POST /api/images/enhance` (OpenAI chat completions, vision-capable model) may ask clarifying questions and then return an enriched prompt. The accepted enriched prompt becomes the input to generation. When reference images are attached, their bytes are sent to the model as base64 data URLs so its critique/questions/enhancement account for what the images already show. The "Edit with reference image" flow still skips enhancement entirely (`suppressEnhanceOnce`).
...
| `POST /api/images/enhance`             | Body: `{ prompt, brandGuidelines?, referenceIds?: string[] }`. Returns `{ critique?: string, clarifyingQuestions?: { question: string; example?: string }[], enhancedPrompt?: string }`. Uses OpenAI chat completions (vision-capable); brand guidelines steer questions/critique; any owned `referenceIds` are loaded and sent to the model as base64 image inputs. |
...
The gallery dialog supports selecting **multiple** assets; confirming materialises each selected asset into a reference and attaches them all to the composer (ImageRouter's edits endpoint accepts up to 16 `image[]` references, model-dependent). Each selected brand asset `POST`s to `/api/images/references/from-brand-asset`, which:
