---
name: channel-revenue-analysis
description: Guidance for analysing Novasero sales revenue by order channel (Dine In, Delivery, Drive Through), payment method, online vs in-store, and time-of-day distribution.
version: 1.0.0
tags:
  - channel
  - revenue
...
- `dim_division_group_channel` on `transactions` — categorises each transaction by order channel.
  Common values: `'Dine In'`, `'Delivery'`, `'Drive Through'`, `'Take Away'`
- `tran_online_factor` — `1` = online order, `0` = in-store
- `cash_method_description` — human-readable payment method (e.g. `'Cash'`, `'Card'`, `'Voucher'`)
- `tran_cash_method` — raw payment method code (use `cash_method_description` for display)
- `receipt_method_description` — order type label (e.g. `'Dine In'`, `'Take Away'`)
...
- When analysing multi-brand channel performance, add `brand` to the GROUP BY and SELECT.
- `store_trading_nature` (e.g. `'Restaurant'`, `'Kiosk'`) can further segment channel results.
- For cross-brand channel comparison, omit the `brand =` filter and group by `brand, dim_division_group_channel`.
